...
def search_and_store_results(query, profile):
    ...
    selector = "#Odp5De > div > div.PKBwyd > div > div.Wz7gbe > block-component > div > div.dG2XIf.XzTjhb > div > div > div > div > div.ifM9O > div > div > div > div > div.g > div > div > div.yuRUbf > div > span > a"
    try:
        ...
        print("url not found for initial preview")
    for i in range(2, 15, 1):
        selector = f"#rso > div:nth-child({i}) > div > div > div.kb0PBd.cvP2Ce.jGGQ5e > div > div > span > a"
        try:
            element = driver.find_element(By.CSS_SELECTOR, selector)
            result_url = element.get_attribute("href")
            if result_url != None:
...
controller_results_1 = search_and_store_results(queries[0], "Profile 3")

# Run queries for the tester profile and store results as t1
tester_results_1 = search_and_store_results(queries[0], "Profile 4")

# Run rest of the queries for the tester profile
tester_results_rest = []
for query in queries[1:]:
    urls = search_and_store_results(query, "Profile 4")
    tester_results_rest.append(urls)

# Run queries for the controller profile again and store results as c2
controller_results_2 = search_and_store_results(queries[0], "Profile 3")

# Run queries for the tester profile again and store results as t2
tester_results_2 = search_and_store_results(queries[0], "Profile 4")

results = {}

results["test1"] = tester_results_1[:10]
results["test2"] = tester_results_2[:10]
results["control1"] = controller_results_1[:10]
results["control2"] = controller_results_2[:10]
